...
import { receiveAccountReceivable } from "@/lib/financial-server"
...
function getErrorMessage(error: unknown) {
  ...
  return "Não foi possível marcar o recebimento."
}
...
function getErrorStatus(error: unknown) {
  ...
  if (error instanceof Error && /(conta|valor|receb)/i.test(error.message)) {
    return 400
  }
...
export async function PATCH(request: NextRequest, { params }: RouteContext) {
  try {
    ...
    const accountId = await receiveAccountReceivable(
      params.id,
